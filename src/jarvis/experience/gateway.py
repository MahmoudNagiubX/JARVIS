...
<main><section class="panel"><h2>SYSTEM STATE</h2><div id="state" class="state">OFFLINE</div><div id="system"></div></section><section class="panel"><h2>RUNS / WORKERS</h2><div id="runs"></div></section><section class="panel"><h2>DEVICES</h2><div id="devices"></div></section><section class="panel"><h2>GOALS / APPROVALS</h2><div id="goals"></div></section><section class="panel"><h2>RESEARCH / ENGINEERING</h2><div id="specialists"></div></section><section class="panel"><h2>INTELLIGENCE</h2><div id="intelligence"></div></section><section class="panel"><h2>VOICE / CONVERSATION</h2><div id="voice"></div></section><section class="panel wide"><h2>EVENT TIMELINE</h2><pre id="timeline">Waiting for authenticated state...</pre></section></main>
<script>
...
function paint(x){document.querySelector('#state').textContent=String(x.state||'offline').toUpperCase();document.querySelector('#system').innerHTML=rows({runtime:x.system?.runtime_state,offline:x.system?.offline,model:x.system?.model_provider||'none',events:x.system?.event_count,errors:x.system?.error_count});document.querySelector('#runs').innerHTML=rows({active:(x.runs||[]).length,worker:(x.system?.active_workers||0),approval:(x.approvals||[]).length});document.querySelector('#devices').innerHTML=rows({registered:(x.devices||[]).length,online:(x.devices||[]).filter(d=>d.status==='online').length});document.querySelector('#goals').innerHTML=rows({goals:(x.goals||[]).length,approvals:(x.approvals||[]).length});document.querySelector('#specialists').innerHTML=rows({research:(x.research||[]).length,engineering:(x.engineering||[]).length});document.querySelector('#intelligence').innerHTML=rows({missions:(x.missions||[]).length,skills:(x.skills||[]).length,automations:(x.automations||[]).length,briefings:(x.briefings||[]).length,findings:(x.intelligence||[]).length,projects:(x.workspace||[]).length,evaluations:(x.evaluations||[]).length});document.querySelector('#voice').innerHTML=rows({state:x.voice?.state||'idle',conversation:x.conversation?.conversation_id||'none'});document.querySelector('#timeline').textContent=(x.timeline||[]).map(e=>`${e.timestamp} ${e.event_type} ${JSON.stringify(e.payload||{})}`).join('\n')||'No events';document.querySelector('#connection').textContent='LIVE / EVENT PROJECTION';document.querySelector('#connection').className='ok'}
async function load(){if(!owner){return}const r=await fetch(`/v1/experience/state?owner_id=${encodeURIComponent(owner)}`);if(r.ok)paint(await r.json())}load();if(owner){const stream=new EventSource(`/v1/experience/events?owner_id=${encodeURIComponent(owner)}`);stream.onmessage=e=>{try{paint(JSON.parse(e.data))}catch(_){}}}
</script></body></html>"""

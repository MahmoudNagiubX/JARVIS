...
|---|---|---|---|
| graphite canvas/surfaces | `ui/src/styles.css`, `MatrixFoundation` | ADOPTED | CSS tokens; no remote runtime dependency |
| electric blue / burgundy / gold / teal semantic palette | existing cyan/crimson variables and state classes | ADAPTED | consolidated product-pass variables; gold remains approval/attention only |
| Lexend / Rubik / JetBrains Mono hierarchy | existing font stacks | ADAPTED | local/system fallbacks; no font download |
| Lucide icon masks | semantic labels plus existing glyph slots in `routes.ts` and shell | DEFERRED P2 | no capability or authority depends on icon assets; bounded icon replacement remains a follow-up, with accessible labels already retained |
...
| `NavRail` | `ContextRail` + `sidebar` in `AppShell` | ADAPTED | five primary areas + Approvals/System utilities; compatibility routes stay reachable |
...
| `Composer` | Chat `textarea` composer | ADAPTED | truthful opening/thinking/approval/verified/unverified states; server remains authoritative |
...
| `Inspector` | no single shared inspector; per-screen details | ADAPTED | contextual Work/Memory/System side panel pattern without a second state authority |
...
| supplied photography | existing owner wallpaper only | REFERENCE + OWNER OVERRIDE | retained only on Command/Home with crop/overlay treatment; not spread across areas |

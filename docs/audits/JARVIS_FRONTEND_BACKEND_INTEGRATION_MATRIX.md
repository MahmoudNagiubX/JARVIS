...
|---|---|---|---|---|---|---|---|---|
| Command | `/`, `HomeScreen`, `AppShell` | `/experience/state`, projection sections | refresh projection | `/experience/events/ws?topic=system_health` | desktop session + cookie | `ExperienceProjection` / EventBus | KEEP as Command | PASS; packaged render reviewed at four viewports |
| Converse | `/chat`, `ChatScreen` | `/conversations`, `/conversations/{id}/messages`, `/runs/{id}`, `/runs/{id}/activity` | `/messages/start`, `/runs/{id}/cancel` | projection/WebSocket plus bounded run polling | desktop session + CSRF for mutations | `AgentRuntime` / conversation service | MOVE to Converse | PASS; live `hi` response survived refresh |
| Work | `/work`, `WorkScreen` | projection missions/research/engineering | links to existing canonical detail screens | projection/EventBus | authenticated owner/device | Goal/Mission, Research, Engineering, Browser authorities | MERGE capability homes | PASS; compatibility detail routes remain reachable |
| Automations | `/automations`, `AutomationsScreen` | `/automations`, projection follow-ups/operations | existing automation/operations endpoints | projection/EventBus | authenticated owner/device + CSRF | Automation, Operations, Notification authorities | KEEP as Automations | PASS; empty/configured states are truthful |
| Missions | `/missions`, `MissionsScreen` | `/missions`, mission detail/evidence | `/missions`, `/missions/{id}/{action}` | projection mission state | authenticated owner | Goal/Mission authority | MOVE under Work → Runs | KEEP compatibility route; Work hub links to canonical screen |
...
| Operations/focus | `/operations`, `OperationsScreen` | `/personal-operations/modes`, `/focus`, projection | `/personal-operations/mode`, `/personal-operations/run`, `/focus/start`, `/focus/end` | projection/event stream | authenticated owner/device + CSRF | Operations/Focus services | MOVE under Work | KEEP compatibility route; Automations links to detail |
...
| Approvals | `/approvals`, `ApprovalsScreen` | projection approvals, `/approvals/{id}` | approval decision endpoints | approval events | authenticated owner/device + CSRF | ApprovalEngine | KEEP as utility | PASS; live owner database had no pending item; pending/decision regressions remain green |
...
| System | `/settings`, `SettingsScreen` | `/health`, `/experience/system`, `/computer/apps`, profile, `/capabilities` | app refresh/settings/computer actions | projection/runtime state | authenticated owner/device + CSRF | runtime/model/desktop authorities | MOVE to System | PASS; packaged System surface showed installed apps and hybrid routing truth |
...
- Native app acceptance remains server-side: the UI only submits text; `AgentRuntime` resolves the narrow intent, `InstalledApplicationRegistry` returns an opaque `app_ref`, and the canonical computer service performs permission, launch, verification, and audit.

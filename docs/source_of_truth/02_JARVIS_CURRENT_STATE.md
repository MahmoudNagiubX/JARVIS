...
| Local visual OCR grounding | `OPEN` (evaluation complete, blocked) | No production capability exists. Batch 04 Milestone 2 evaluated PaddleOCR (crashes on every prediction with a reproducible PaddlePaddle CPU oneDNN executor bug on this machine) and RapidOCR (runs, but its only Arabic recognition tier scores far under the required accuracy gate; also independently reproduced a known undeclared `python-bidi` dependency risk) - both failed acceptance gates. Verdict: `OCR_BACKEND_EVALUATION_BLOCKED`, no unsafe integration forced. See `docs/audits/PHASE_18_WORKSTREAM_A_BATCH_04.md` Section 5 and `05_JARVIS_DECISION_LOG.md` OPEN-002 for full evidence. Benchmark tooling preserved at `scripts/phase18/ocr_backend_benchmark.py` for a future re-evaluation. |

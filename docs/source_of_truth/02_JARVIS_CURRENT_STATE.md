...
| 18 | Phase 18A (baseline audit + targeted stabilization) complete as of 2026-09-12; Phase 18 capability workstreams (A–H) not yet started |
...
| Computer Use V2 evaluation suite | `PARTIAL` | deterministic `computer_use_v2` suite (`src/jarvis/evaluation/computer_use_v2.py`, 32 cases as of Batch 04 Milestone 1, up from 24) registered through the existing `EvaluationService` - no GUI dependency, all cases passing. Opt-in physical acceptance runner `scripts/phase18/computer_use_acceptance.py` (not part of production startup) exercises two JARVIS-owned native Win32 fixtures together (never an owner app); run 3/3 on NIGHTFURY across semantic invoke/toggle/select, native left/right/double-click, scroll, Tab-key focus, named chords, a non-primary-monitor click, grounded drag, English/Arabic literal typing, Home/End/Backspace, and clipboard proof - every scenario 3/3. Does not yet cover the full real-app/failure-mode breadth in GAP-0105. |
| Local visual OCR grounding | `PARTIAL` (read-only only - no visual actuation) | Batch 04 Milestone 2 evaluated PaddleOCR (crashed on every prediction - PaddlePaddle CPU oneDNN executor bug) and RapidOCR (Arabic recognition tier scores far under the accuracy gate) - both failed, `OCR_BACKEND_EVALUATION_BLOCKED`, no integration forced. Batch 05 Milestone 1 resolved it: re-ran RapidOCR with corrected Arabic fixture shaping (still fails, confirming a genuine model limitation, not a rendering artifact) and evaluated a third candidate, **EasyOCR 1.7.2** (PyTorch CPU, no system binary/installer), which passed every gate (1.0 normalized recall on all English/Arabic/mixed fixtures, 3/3 reproducible runs, DEC-048). Production integration: `computer.visual.read` (`ocr_window`/`ocr_element`), grounded strictly by opaque `window_ref`/`element_ref`, behind `EasyOcrVisualAdapter` (`src/jarvis/computer/visual_ocr.py`) reusing the existing GDI capture and privacy/staleness checks (no second screenshot subsystem, no second sensitivity policy). Optional `computer-ocr` dependency group; core JARVIS and every other Computer Use capability work unchanged when it is absent. Bounded output (100 regions/512 chars per region/12,000 chars total, confidence clamped 0..1), memory-only screenshot bytes released immediately after OCR, opaque `visual-<uuid>` references (20s TTL) that are strictly observation-only this batch - no visual actuation exists anywhere. Physically proven end to end against the owned Win32 fixture (`ocr_window` + `ocr_element`, 3/3 clean runs, real GDI capture + real EasyOCR inference, honestly mixed confidence). See `docs/audits/PHASE_18_WORKSTREAM_A_BATCH_05.md` and `05_JARVIS_DECISION_LOG.md` DEC-048 for full evidence. Batch 06 Milestone 0 hardened production OCR to be offline-only by construction (`download_enabled=False`, explicit product-owned model directory, no implicit `~/.EasyOCR` fallback, verified before `Reader()` is ever constructed) and pinned the exact CPU-only `torch`/`torchvision` runtime alongside `easyocr` for reproducibility; model acquisition moved to a separate, never-production-triggered setup script (`scripts/setup/provision_easyocr_models.py`). Milestone 1 closed the previously-flagged missing unified live-GUI Arabic physical evidence gap: a fourth owned fixture rendering real Arabic Unicode through Windows' own live text shaping was recognized with an exact match, 3/3 runs, through the real production tool path, with zero network access. See `docs/audits/PHASE_18_WORKSTREAM_A_BATCH_06.md` for full evidence. |

...
| UIA semantic control tree/actions | `PARTIAL` | read (`computer.semantic.read`) and bounded write (`computer.semantic.act`: invoke/toggle/select, approval-required by default) tool paths implemented through `ComputerActionService`, when the optional `computer-uia` dependency is installed (DEC-046; Batch 01 Milestones 1-3, Batch 02 Milestone 0 of Phase 18 Workstream A). `invoke` physically proven live on NIGHTFURY (Calculator "Seven" button, display changed 0→7 through the real approval-gated action path, re-confirmed through the Batch 02 hardened pipeline); `toggle`/`select` are proven by deterministic unit tests only - no live-NIGHTFURY control exposing `TogglePattern`/`SelectionItemPattern` was available on the disposable apps used across both batches (a disposable Edge Guest + local HTML fixture attempt in Batch 02 found Chromium page content sits deeper than the adapter's existing `MAX_INSPECT_DEPTH=5` bound). Batch 02 Milestone 0 additionally hardened targeting: explicit strong/weak identity (weak refs never actionable), `list_windows` privacy filtering, fresh post-action re-observation (never the pre-action object/pattern), a target-aware time-bounded approval preview (refuses approval creation for stale/weak/non-actionable targets, detects target drift on decide, expiry bounded by the element-ref TTL), and explicit disabled/offscreen/password fail-closed checks before actuation - see `docs/audits/PHASE_18_WORKSTREAM_A_BATCH_02.md`. No generic text/value write (`set_value`), no OCR/visual fallback, no multi-app recovery loop - see GAP-0102 through GAP-0106. |

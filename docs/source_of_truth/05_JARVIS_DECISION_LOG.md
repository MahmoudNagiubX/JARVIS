...
|---|---|---|
| OPEN-002 | exact OCR/visual grounding model/adapter | evaluate after semantic UIA baseline; must fit hardware/privacy. **Batch 04 Milestone 2 evaluation (2026-09-13, NIGHTFURY, Python 3.12.10 isolated venv):** both evaluated candidates failed the acceptance gates - **not** locked, still open. PaddleOCR 3.7.0 + paddlepaddle 3.3.1 (PP-OCRv5, CPU) crashes on every single prediction with a reproducible `NotImplementedError: ConvertPirAttribute2RuntimeAttribute not support [...] onednn_instruction.cc` deep inside PaddlePaddle's CPU oneDNN executor; unresolved after disabling oneDNN via `FLAGS_use_mkldnn` (env var and programmatic `paddle.set_flags`) and after downgrading paddlepaddle to 2.6.2 (which then breaks paddleocr 3.7.0's own API compatibility instead - `AnalysisConfig.set_optimization_level` missing). RapidOCR 3.9.2 (secondary candidate, CPU/onnxruntime) runs, but (a) its only available Arabic recognition tier for either PP-OCRv4 or PP-OCRv5 is "mobile" (no larger tier exists), scoring far under the required 0.85 normalized recall on the fixture corpus (observed 0.0-0.41 per case); (b) confirmed empirically the exact upstream risk this task named in advance - a clean `pip install rapidocr` has no declared dependency on `python-bidi`, and Arabic recognition raises `ModuleNotFoundError` until it is installed manually. English recall was excellent (~1.0) under RapidOCR's *default* PP-OCRv6 "small" tier but poor (~0.35-0.41) under the "mobile" tier forced to match Arabic - the only tier where both languages are simultaneously available. Full per-case evidence, exact versions, and footprint in `docs/audits/PHASE_18_WORKSTREAM_A_BATCH_04.md` Section 5; benchmark script preserved at `scripts/phase18/ocr_backend_benchmark.py`. No production OCR integration exists; GAP-0103 stays `OPEN`. |

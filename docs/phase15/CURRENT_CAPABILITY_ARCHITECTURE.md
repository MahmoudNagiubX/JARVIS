...
4. `BrowserActionService` applies permission/approval/audit/event behavior around `LocalBrowserController` or an injected Playwright controller; browser tools enter through this same service and the local controller applies the shared URL/redirect policy.
...
MCP input schemas are normalized to a bounded JARVIS-owned structural subset
before `MCPDiscoveredTool` registration and remain bounded when converted to
`ToolSpec` model-facing schemas. The MCP health report separately identifies
the foundation, product-owned local servers, and optional external STDIO state.

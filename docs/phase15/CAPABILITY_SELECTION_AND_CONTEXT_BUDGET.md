...
the existing native registry, including namespaced MCP tools, using normalized
user intent and a maximum of eight tools. MCP schemas are also bounded by a
32,000-byte aggregate budget.
...
names, schema byte count, and a safe selection reason for observability.
